...
static void cd_mpris2_get_time_elapsed (void)
{
	...
	GVariant *res = g_dbus_connection_call_sync (g_dbus_proxy_get_connection (myData.pProxyPlayer),
		myData.pCurrentHandler->cMpris2Service, CD_MPRIS2_OBJ, "org.freedesktop.DBus.Properties",
	...
	switch (pControl)
	{
		...
		case PLAYER_SHUFFLE :
			...
			g_dbus_connection_call (g_dbus_proxy_get_connection (myData.pProxyPlayer),
				myData.pCurrentHandler->cMpris2Service, CD_MPRIS2_OBJ, "org.freedesktop.DBus.Properties", "Set",
		...
		case PLAYER_ENQUEUE :
		{
			...
			if (s_bHasTrackList) // note: we don't need a separate proxy to the TrackList interface
				g_dbus_connection_call (g_dbus_proxy_get_connection (myData.pProxyPlayer),
					myData.pCurrentHandler->cMpris2Service, CD_MPRIS2_OBJ, "org.mpris.MediaPlayer2.TrackList",
...
static void cd_mpris2_start (void)
{
	...
	g_dbus_proxy_new_for_bus (G_BUS_TYPE_SESSION,
		...
		myData.pCurrentHandler->cMpris2Service, // maybe this should be a parameter
